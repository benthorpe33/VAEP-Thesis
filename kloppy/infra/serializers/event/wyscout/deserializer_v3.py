...
formations = {
    "4-3-2": FormationType.FOUR_THREE_TWO,
    "5-3-1": FormationType.FIVE_THREE_ONE,
    "4-1-3-1": FormationType.FOUR_ONE_THREE_ONE,
    "4-4-1": FormationType.FOUR_FOUR_ONE,
    "4-2-1-2": FormationType.FOUR_TWO_ONE_TWO,
...
def _parse_pass(raw_event: Dict, next_event: Dict, team: Team) -> Dict:
    ...
    if raw_event["pass"]["accurate"] is True:
        ...
        receiver_player = raw_event["pass"]["recipient"]
...
def get_home_away_team_formation(event, team, home_bool):
    if home_bool:
...
def identify_synthetic_formation_change_event(
    ...
    if current_event_team == home_team:
        current_is_home = True
    else:
        current_is_home = False
    if next_event_team == home_team:
        next_is_home = True
    else:
        next_is_home = False
    ...
    ) = get_home_away_team_formation(raw_event, current_event_team, current_is_home)
    (
        next_home_team_formation,
        next_away_team_formation,
    ) = get_home_away_team_formation(raw_next_event, next_event_team, next_is_home)
...
class WyscoutDeserializerV3(EventDataDeserializer[WyscoutInputs]):
    ...
    def deserialize(self, inputs: WyscoutInputs) -> EventDataset:
        ...
        with performance_logging("parse data", logger=logger):
            ...
            home_team, away_team = raw_events["teams"].values()
            teams = {home_team_id: home_team, away_team_id: away_team}
            players = raw_events["players"]
            # players = dict(
            #     [
            ...
            for idx, raw_event in enumerate(raw_events["events"]):
                ...
                if next_event:
                    ...
                    for (
                        ...
                        if event and self.should_include_event(event):
                            events.append(transformer.transform_event(event))
                if isinstance(event.team, str):
                    event.team = {"team_id": team_id, "team_name": team_name}
        home_team = {"home_team": home_team, "home_team_id": home_team_id}
        away_team = {"away_team": away_team, "away_team_id": away_team_id}
        metadata = Metadata(
